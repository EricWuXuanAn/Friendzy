...
class CompanionMyPublishVM() : ViewModel() {
    ...
    //回傳值
    private val _addReturnState = MutableStateFlow(0)
    val addReturnState = _addReturnState.asStateFlow()

    //修改_addReturnState的值
    fun reAddReturn() {
        _addReturnState.update { 0 }
    }

    //寫入刊登資料
    fun setPublish(myPublish: MyPublish) {
        viewModelScope.launch {
            _setMyPublishState.update { myPublish }
        }
    }

    fun addPublish(myPublish: MyPublish) {
        viewModelScope.launch {
            val response = addServiceAndOrder(myPublish)
            Log.e("_tagPublish1 response", response.toString())
            _addReturnState.update { response }
        }
    }
    ...
    fun getAreaState() {
        viewModelScope.launch {
            val response = fetchAreaList()
            _publishAreaState.update { response }
        }
    }
    ...
    //用刊登資料接API新增服務、訂單各一筆
    private suspend fun addServiceAndOrder(myPublish: MyPublish): Int {
        try {
